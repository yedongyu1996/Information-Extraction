...
def _beam_search_generate(decoder: Seq2SeqDecoder, tokens=None, state=None, max_length=20, max_len_a=0.0, num_beams=4,
    ...
    """
    bsz, 2 + len(src_len) + len(mapping)
    为什么不是bsz, len_tokens, 2 + len(src_len) + len(mapping)
    因为在生成的时候，只要知道最后一个时刻的输出
    """
    ...
    if restricter is not None:
        ...
        # 返回最大的几个值 _next_scores == values  _next_tokens == indices, 可以这么理解， 假设bsz=1， 取出的_next_scores代表最大几个tokens的概率，_next_tokens就是哪几个token
    ...
    keep_mask = not_eos_mask.cumsum(dim=1).le(num_beams)  # 为1的地方需要保留 ， le比较， 小于或者等于
    keep_mask = not_eos_mask.__and__(keep_mask)  # 为1的地方是需要进行下一步search的

    next_tokens = _next_tokens.masked_select(keep_mask).view(batch_size, num_beams)  # 这是真的接下来要继续的， mask_select 把keep_mask中是1的位置的next_tokens拿掉
    next_scores = _next_scores.masked_select(keep_mask).view(batch_size, num_beams)  # mask_select 把keep_mask中是1的位置的next_tokens拿掉（因为eos是1？）

    rows, cols = not_eos_mask.eq(0)[:, :num_beams].nonzero(as_tuple=True)  # 返回非零元素的index， 分别从行列来看
