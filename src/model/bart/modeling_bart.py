...
_TOKENIZER_FOR_DOC = './data/vocab.txt'
# _TOKENIZER_FOR_DOC = r'D:\深度学习\端到端模型抽取\BARTABSA-main\BARTABSA-main\ye_aoesc\data\vocab.txt'
# _TOKENIZER_FOR_DOC = r'/home/njupt/ydy/BARTABSA-main/BARTABSA-main/ye_aoesc/data/vocab.txt'
...
class BartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        # self.max_source_positions = 514
        self.max_source_positions = 1026

        self.embed_tokens = embed_tokens
        if config.static_position_embeddings:
            self.embed_positions = SinusoidalPositionalEmbedding(
                # config.max_position_embeddings, embed_dim, self.padding_idx
                # 514, embed_dim, self.padding_idx
                1026, embed_dim, self.padding_idx
            )
        else:
...
class BartDecoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens: nn.Embedding):
        ...
        # self.max_target_positions = 514
        self.max_target_positions = 1026
        self.embed_scale = math.sqrt(config.d_model) if config.scale_embedding else 1.0
        self.embed_tokens = embed_tokens
        if config.static_position_embeddings:
            self.embed_positions = SinusoidalPositionalEmbedding(
                # config.max_position_embeddings, config.d_model, config.pad_token_id
                # 514, config.d_model, config.pad_token_id
                1026, config.d_model, config.pad_token_id
            )
        else:
            self.embed_positions = LearnedPositionalEmbedding(
                # config.max_position_embeddings,
